package controllers;
	...
	public class GraphHumAirController  implements Initializable{
		...
		private void iniLineChart2() throws NumberFormatException, IOException {
	        ...
	        BufferedReader inputStream = null;
	            ...
	            int m=0;
	             try {
	                  inputStream = new BufferedReader(new FileReader("D:\\MIOLA\\smartgreen_house\\fileHumAir.txt"));
	...
	if(m==4){
	 series.getData().add(new XYChart.Data("50s",c));
	  ...
	  if(m==6){
	      ...
	      series.getData().add(new XYChart.Data("80s",c));  
          System.out.println("la val "+c);
  }if(m==8){
      series.getData().add(new XYChart.Data("90s",c));  
      System.out.println("la val "+c);
}m++;
	                                     }
	                            } finally {
